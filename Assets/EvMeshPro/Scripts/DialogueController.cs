...
public class DialogueController : MonoBehaviour
{
    ...
    [FormerlySerializedAs("_delayAfterEndPhrase")] [SerializeField]
    private float delayAfterEndPhrase = 1f;
    ...
    public static event Action OnEndPhrase;
    ...
    private void MouseDown(Textbox textbox)
    {
        if (firstPress)
        {
            textbox.StopSpawn();
            textbox.currentCoroutine = null;
            textbox.DisplayText(0);
            firstPress = false;
            return;
        }
    ...
    private IEnumerator IterateQue()
    {
        ...
        float displayLength = currentTextBox.dialogue.Split(' ').Length * textTypeSpeed * 6 + delayAfterEndPhrase;
        ...
        if (!currentTextBox.IsButton)
        {
            yield return new WaitForSeconds(displayLength);

            var dialogue = dialogueInstanceQue[0].GetComponent<Textbox>();
            if (dialogue != null)
            {
                if (currentTextBox == dialogue)
                {
                    EndDialogue();
                }
            }
